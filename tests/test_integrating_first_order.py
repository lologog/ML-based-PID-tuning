...
def test_positive_gain():
    ...
    assert output == pytest.approx(0.0039215686)

def test_zero_gain():
    plant = IntegratingFirstOrderPlant(gain=0.0, time_constant=5.0)
...
def test_nan_gain():
    try:
        plant = IntegratingFirstOrderPlant(gain=float("nan"), time_constant=5.0)
        assert False
...
def test_positive_infinity_gain():
    try:
        plant = IntegratingFirstOrderPlant(gain=float("inf"), time_constant=5.0)
        assert False
...
def test_zero_time_constant():
    try:
        plant = IntegratingFirstOrderPlant(gain=2.0, time_constant=0.0)
        assert False
...
def test_nan_time_constant():
    try:
        plant = IntegratingFirstOrderPlant(gain=2.0, time_constant=float("nan"))
        assert False
...
def test_positive_infinity_time_constant():
    try:
        plant = IntegratingFirstOrderPlant(gain=2.0, time_constant=float("inf"))
        assert False
