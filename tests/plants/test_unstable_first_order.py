...
def test_positive_gain():
    ...
    assert output == pytest.approx(0.0408163265)

def test_zero_gain():
    plant = UnstableFirstOrderPlant(gain=0.0, time_constant=5.0)
...
def test_nan_gain():
    try:
        plant = UnstableFirstOrderPlant(gain=float("nan"), time_constant=5.0)
        assert False
...
def test_positive_infinity_gain():
    try:
        plant = UnstableFirstOrderPlant(gain=float("inf"), time_constant=5.0)
        assert False
...
def test_zero_time_constant():
    try:
        plant = UnstableFirstOrderPlant(gain=2.0, time_constant=0.0)
        assert False
...
def test_nan_time_constant():
    try:
        plant = UnstableFirstOrderPlant(gain=2.0, time_constant=float("nan"))
        assert False
...
def test_positive_infinity_time_constant():
    try:
        plant = UnstableFirstOrderPlant(gain=2.0, time_constant=float("inf"))
        assert False
...
def test_dt_equal_to_time_constant():
    plant = UnstableFirstOrderPlant(gain=2.0, time_constant=5.0)

    try:
        plant.update(input_signal=1.0, dt=5.0)
        assert False

    except ValueError as error:
        assert str(error) == "dt must not be equal to time_constant"
